...
describe('SettingsScreen', () => {
  ...
  it('hides PromotionBanner when user is on full version', async () => {
  ...
  it('logs settings view analytics event on focus', async () => {
    await render(<SettingsScreen />);

    expect(AnalyticsService.logSettingsViewEvent).toHaveBeenCalled();
  });

  it('calls openTermsOfService when terms item is pressed', async () => {
  ...
  it('calls openPrivacyPolicy when privacy item is pressed', async () => {
    const { useWebPagesNavigation } = jest.requireMock('@/hooks/navigation/useWebPagesNavigation');
