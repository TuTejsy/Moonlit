...
description: Constraints and guidelines for screen and modal headers, scroll integration, and hitSlops in Moonlit.
globs: 'src/components/Headers/**/*, src/screens/**/*.tsx, src/components/**/*.tsx'
---

# Header Components (`src/components/Headers/`)

The project uses a set of standard header components to ensure visual and interactive consistency across screens.
...
- **Shared Constants**:
  - **`EXTRA_TOUCH_AREA`**: Always use this constant (imported from `Headers.constants.ts`) for `hitSlop` on header buttons to ensure accessible touch targets (16dp inset).
